...
# run container (host network recommended so ONVIF discovery works)
# specify a unique --mac-address per camera if needed
...
Run additional containers with different names and MAC addresses or define several streams in one container. To run separate containers use docker compose:
